...
def run_empire(name, tab_file_path, result_file_path, scenariogeneration, scenario_data_path,
	...
	def prepInvCost_rule(model):
		...
		for i in model.Period:
			...
			costperperiod=costperyear(1-(1+model.discountrate)**-(min(value((9-i+1)*5),model.offshoreConvLifetime)))/(1-(1/(1+model.discountrate)))
	...
	if optimizingWithBinaries is True:
		model.offshoreHubFixedCostPaid = Var(model.OffshoreEnergyHubs, domain=Binary)
	...
	if hydrogen is True:
		...
		#Hydrogen variables
		#Operational
		model.hydrogenProduced = Var(model.HydrogenProdNode, model.Operationalhour, model.Period, model.Scenario, domain=NonNegativeReals)
		...
		model.powerForHydrogen = Var(model.HydrogenProdNode, model.Operationalhour, model.Period, model.Scenario, domain=NonNegativeReals)

		#This hydrogen market is not yet connected to any particular node -- sidestepping the problem of "distance" to other markets, and building pipelines to these.
		model.hydrogenSoldOtherMarket = Var(model.HydrogenProdNode, model.Operationalhour, model.Period, model.Scenario, domain=NonNegativeReals)
		...
		model.hydrogenForPower = Var(model.HydrogenProdNode, model.Operationalhour, model.Period, model.Scenario, domain=NonNegativeReals)
		...
		def prepPipelineInvcost_rule(model):
			for i in model.Period:
				for (n1,n2) in model.HydrogenBidirectionPipelines:
					...
					costperperiod =costperyear*(1-(1+model.discountrate)**-(min(value((9-i+1)*5), value(model.hydrogenPipelineLifetime))))/(1-(1/(1+model.discountrate)))
		...
		if h2storage is True:
			def prepHydrogenStorageInvcost_rule(model):
				for i in model.Period:
					...
					costperperiod = costperyear*1000*(1-(1+model.discountrate)**-(min(value((9-i+1)*5), value(model.hydrogenStorageLifetime))))/(1-(1/(1+model.discountrate)))
	...
	if hydrogen is True:
		def hydrogen_operational_storage_cost_rule(model,i):
			return sum(model.operationalDiscountrate*model.seasScale[s]*model.sceProbab[w]*model.hydrogenProduced[n,h,i,w]*model.averageHydrogenSeasonalStorageCost for n in model.HydrogenProdNode for (s,h) in model.HoursOfSeason for w in model.Scenario)
		model.hydrogen_operational_storage_cost = Expression(model.Period, rule=hydrogen_operational_storage_cost_rule)

		def hydrogen_income_other_markets_rule(model,i):
			return sum(model.operationalDiscountrate*model.seasScale[s]*model.sceProbab[w]*model.hydrogenSoldOtherMarket[n,h,i,w]*model.hydrogenPriceOtherMarkets for n in model.HydrogenProdNode for (s,h) in model.HoursOfSeason for w in model.Scenario)
		model.hydrogen_income_other_markets = Expression(model.Period, rule=hydrogen_income_other_markets_rule)
	#############
	##OBJECTIVE##
	#############
	...
	def Obj_rule(model):
		if hydrogen is True:
			returnSum = sum(model.discount_multiplier[i]*(sum(model.genInvCost[g,i]* model.genInvCap[n,g,i] for (n,g) in model.GeneratorsOfNode ) + \
														  ...
														  model.hydrogen_operational_storage_cost[i] - model.hydrogen_income_other_markets[i])
							for i in model.Period)
			if h2storage is True:
	...
	def installed_trans_cap_rule(model, n1, n2, i):
		if optimizingWithBinaries is True:
			if n1 in model.OffshoreEnergyHubs:
				return model.transmissionInstalledCap[n1,n2,i] - model.transmissionMaxInstalledCap[n1,n2,i] * model.offshoreHubFixedCostPaid[n1] <= 0
			elif n2 in model.OffshoreEnergyHubs:
				return model.transmissionInstalledCap[n1,n2,i] - model.transmissionMaxInstalledCap[n1,n2,i] * model.offshoreHubFixedCostPaid[n2] <= 0
			else:
	...
	if optimizingWithBinaries is True:
		def only_one_hub_rule(model):
			return sum(model.offshoreHubFixedCostPaid[n] for n in model.OffshoreEnergyHubs) <= 1
		model.only_one_hub = Constraint(rule=only_one_hub_rule)

	def powerFromHydrogenRule(model, n, g, h, i, w):
		if g == "HydrogenOCGT" or g == "HydrogenCCGT":
			if hydrogen is True and n is model.HydrogenProdNode:
				return model.genOperational[n,g,h,i,w] == model.genEfficiency[g,i] * model.hydrogenForPower[n,h,i,w] * model.hydrogenLHV
			else:
	...
	if hydrogen is True:
		def lifetime_rule_pipeline(model,n1,n2,i):
			startPeriod = 1
			if value(1+i-model.hydrogenPipelineLifetime*(1/model.LeapYearsInvestment))>startPeriod:
		...
		def lifetime_rule_elyzer(model,n,i):
			startPeriod = 1
			if value(1+i-model.elyzerLifetime*(1/model.LeapYearsInvestment))>startPeriod:
		...
		def hydrogen_flow_balance_rule(model,n,h,i,w):
			if h2storage is True:
				if n in model.HydrogenProdNode:
					return model.hydrogenProduced[n,h,i,w] \
						   - model.hydrogenSold[n,h,i,w] \
						   - model.hydrogenForPower[n,h,i,w] \
				...
				if n in model.HydrogenProdNode:
					return model.hydrogenProduced[n,h,i,w] \
						   - model.hydrogenSold[n,h,i,w] \
						   - model.hydrogenSoldOtherMarket[n,h,i,w] \
						   - model.hydrogenForPower[n,h,i,w] \
						   + sum(model.hydrogenSent[(n2,n),h,i,w] - model.hydrogenSent[(n,n2),h,i,w] for n2 in model.HydrogenLinks[n]) \
						   == 0
				else:
		...
		def hydrogen_only_sell_other_markets_in_germany_rule(model,n,h,i,w):
			if n in otherMarketNodes:
				return Constraint.Skip
			else:
				return model.hydrogenSoldOtherMarket[n,h,i,w] == 0
		model.hydrogen_only_sell_other_markets_in_germany = Constraint(model.HydrogenProdNode, model.Operationalhour, model.Period, model.Scenario, rule=hydrogen_only_sell_other_markets_in_germany_rule)

		def hydrogen_production_rule(model,n,h,i,w):
			return model.hydrogenProduced[n,h,i,w] == model.powerForHydrogen[n,h,i,w] / model.elyzerPowerConsumptionPerKg[i]
		...
		def hydrogen_production_capacity_rule(model,n,h,i,w):
			return model.powerForHydrogen[n,h,i,w] <= model.elyzerTotalCap[n,i]
		model.hydrogen_production_capacity = Constraint(model.HydrogenProdNode, model.Operationalhour, model.Period, model.Scenario, rule=hydrogen_production_capacity_rule)
	...
	if hydrogen is True:
		print("Hydrogen price in other markets: " + str(h2priceOtherMarkets))
	print("--------------------------------------------------------------\n")
...
#	print(opt.version())
	...
	for (n1,n2) in instance.BidirectionalArc:
		if optimizingWithBinaries is True:
			if n1 in instance.OffshoreEnergyHubs:
				for i in instance.Period:
					writer.writerow([n1,n2,inv_per[int(i-1)],
									 value(instance.transmissionInvCap[n1,n2,i]), value(instance.transmissionMaxBuiltCap[n1,n2,i]) * value(instance.offshoreHubFixedCostPaid[n1]),
									 value(instance.transmissionInstalledCap[n1,n2,i]), value(instance.transmissionMaxInstalledCap[n1,n2,i]) * value(instance.offshoreHubFixedCostPaid[n1]),
			...
			elif n2 in instance.OffshoreEnergyHubs:
				for i in instance.Period:
					writer.writerow([n1,n2,inv_per[int(i-1)],
									 value(instance.transmissionInvCap[n1,n2,i]), value(instance.transmissionMaxBuiltCap[n1,n2,i]),
									 value(instance.transmissionInstalledCap[n1,n2,i]), value(instance.transmissionMaxInstalledCap[n1,n2,i]) * value(instance.offshoreHubFixedCostPaid[n2]),
									 value(instance.discount_multiplier[i]*instance.transmissionInvCap[n1,n2,i]*instance.transmissionInvCost[n1,n2,i]) * value(instance.offshoreHubFixedCostPaid[n2]),
									 value(sum(instance.sceProbab[w]*instance.seasScale[s]*(instance.transmissionOperational[n1,n2,h,i,w]+instance.transmissionOperational[n2,n1,h,i,w])/1000 for (s,h) in instance.HoursOfSeason for w in instance.Scenario)),
									 value(sum(instance.sceProbab[w]*instance.seasScale[s]*((1 - instance.lineEfficiency[n1,n2])*instance.transmissionOperational[n1,n2,h,i,w] + (1 - instance.lineEfficiency[n2,n1])*instance.transmissionOperational[n2,n1,h,i,w])/1000 for (s,h) in instance.HoursOfSeason for w in instance.Scenario))])
			else:
				for i in instance.Period:
	...
	writer.writerow(header)
	if optimizingWithBinaries is False:
		for i in instance.Period:
			for w in instance.Scenario:
		...
		for i in instance.Period:
			for w in instance.Scenario:
				my_string=[inv_per[int(i-1)],w,
						   value(sum(instance.seasScale[s]*instance.genOperational[n,g,h,i,w]*instance.genCO2TypeFactor[g]*(3.6/instance.genEfficiency[g,i]) for (n,g) in instance.GeneratorsOfNode for (s,h) in instance.HoursOfSeason))]
				if EMISSION_CAP:
					my_string.append("NaN: Cannot evaluate duals when binaries are enabled")
				else:
					my_string.append(value(instance.CO2price[i,w]))
				my_string.extend([value(instance.CO2cap[i]*1e6),
								  value(sum(instance.seasScale[s]*instance.genOperational[n,g,h,i,w]/1000 for (n,g) in instance.GeneratorsOfNode for (s,h) in instance.HoursOfSeason)),
								  value(sum(instance.seasScale[s]*instance.genOperational[n,g,h,i,w]*instance.genCO2TypeFactor[g]*(3.6/instance.genEfficiency[g,i]) for (n,g) in instance.GeneratorsOfNode for (s,h) in instance.HoursOfSeason)/sum(instance.seasScale[s]*instance.genOperational[n,g,h,i,w] for (n,g) in instance.GeneratorsOfNode for (s,h) in instance.HoursOfSeason)),
								  value("NaN: Cannot evaluate duals when binaries are enabled"),
								  value(sum(instance.seasScale[s]*(instance.genCapAvail[n,g,h,w,i]*instance.genInstalledCap[n,g,i] - instance.genOperational[n,g,h,i,w])/1000 for (n,g) in instance.GeneratorsOfNode if g == 'Hydrorun-of-the-river' or g == 'Windonshore' or g == 'Windoffshore' or g == 'Solar' for (s,h) in instance.HoursOfSeason)),
								  value(sum(instance.seasScale[s]*((1 - instance.storageDischargeEff[b])*instance.storDischarge[n,b,h,i,w] + (1 - instance.storageChargeEff[b])*instance.storCharge[n,b,h,i,w])/1000 for (n,b) in instance.StoragesOfNode for (s,h) in instance.HoursOfSeason)),
								  value(sum(instance.seasScale[s]*((1 - instance.lineEfficiency[n1,n2])*instance.transmissionOperational[n1,n2,h,i,w] + (1 - instance.lineEfficiency[n2,n1])*instance.transmissionOperational[n2,n1,h,i,w])/1000 for (n1,n2) in instance.BidirectionalArc for (s,h) in instance.HoursOfSeason))])
				writer.writerow(my_string)
	...
	for g in instance.Generator:
		for i in instance.Period:
			writer.writerow([g,inv_per[int(i-1)],value(sum(instance.genInvCap[n,g,i] for n in instance.Node if (n,g) in instance.GeneratorsOfNode)),
							 ...
							 value(sum(instance.seasScale[s]*instance.sceProbab[w]*instance.genOperational[n,g,h,i,w]/1000 for n in instance.Node if (n,g) in instance.GeneratorsOfNode for (s,h) in instance.HoursOfSeason or w in instance.Scenario))])
	writer.writerow([""])
	...
	if hydrogen is True:
		...
		writer = csv.writer(f)
		my_header = ["Node","Period","New capacity [MW]", "Total capacity [MW]", "New capacity [kg/h]", "Total capacity [kg/h]",
					 "Expected annual power usage [MWh]","Expected annual hydrogen production [kg]", "Expected marginal price of hydrogen [EUR/kg]"]
		writer.writerow(my_header)
			...
			for i in instance.Period:
				try:
					writer.writerow([n,inv_per[int(i-1)],
									 ...
									 value(instance.elyzerCapBuilt[n,i] / instance.elyzerPowerConsumptionPerKg[i]),
									 value(instance.elyzerTotalCap[n,i] / instance.elyzerPowerConsumptionPerKg[i]),
									 value(sum(instance.seasScale[s] * instance.sceProbab[w] * instance.powerForHydrogen[n,h,i,w] for (s,h) in instance.HoursOfSeason for w in instance.Scenario)),
									 value(sum(instance.seasScale[s] * instance.sceProbab[w] * instance.powerForHydrogen[n,h,i,w] for (s,h) in instance.HoursOfSeason for w in instance.Scenario) / instance.elyzerPowerConsumptionPerKg[i]),
									 value(sum(instance.sceProbab[w]*instance.dual[instance.meet_hydrogen_demand[n,i,w]] for w in instance.Scenario))])
				except:
					#print('Something went wrong when accessing dual for meet_hydrogen_demand with key for results_hydrogen_investments.csv. Key (node,period,scenario)=('+n+','+str(i)+','+w+')')
					fError.write('Something went wrong when accessing dual for meet_hydrogen_demand with key for results_hydrogen_investments.csv. Key (node,period,scenario)=('+n+','+str(i)+','+w+'). Dual is likely 0 (poorly handled in Xpress)\n')
					writer.writerow([n, inv_per[int(i - 1)],
									 value(instance.elyzerCapBuilt[n, i]),
									 value(instance.elyzerTotalCap[n, i]),
									 value(instance.hydrogenKgPerMWh * instance.elyzerCapBuilt[n, i]),
									 value(instance.hydrogenKgPerMWh * instance.elyzerTotalCap[n, i]),
									 value(sum(instance.seasScale[s] * instance.sceProbab[w] * instance.powerForHydrogen[n, h, i, w] for (s, h) in instance.HoursOfSeason for w in instance.Scenario)),
									 value(instance.hydrogenKgPerMWh * sum(instance.seasScale[s] * instance.sceProbab[w] * instance.powerForHydrogen[n, h, i, w] for (s, h) in instance.HoursOfSeason for w in instance.Scenario)),
									 0])
		if solver == 'Xpress':
			fError.write('\n')
			fError.close()
		...
		writer = csv.writer(f)
		my_header = ["Node", "Period", "Scenario", "Season", "Hour", "Power for hydrogen [MWh]", "Hydrogen produced [kg]"]
		writer.writerow(my_header)
		for n in instance.HydrogenProdNode:
			for i in instance.Period:
				for w in instance.Scenario:
					for (s,h) in instance.HoursOfSeason:
						my_string = [n, inv_per[int(i-1)], w, s, h,
									 ...
									 value(instance.hydrogenProduced[n,h,i,w])]
						writer.writerow(my_string)
		...
		if solver == 'Xpress':
			fError = open(result_file_path + "/" + "errorLog.log",'a')
		writer = csv.writer(f)
		my_header = ["Node", "Period", "Scenario", "Season", "Hour", "Hydrogen sold [kg]", "Hydrogen stored [kg]", "Hydrogen withdrawn from storage [kg]", "Total demand in period [kg]", "Hydrogen burned for power [kg]", "Hydrogen price [EUR]", 'Hydrogen sold on other market']
		writer.writerow(my_header)
		if h2storage is True:
			for n in instance.HydrogenProdNode:
				for i in instance.Period:
					for w in instance.Scenario:
						...
						for (s, h) in instance.HoursOfSeason:
							my_string = [n, inv_per[int(i - 1)], w, s, h,
										 ...
										 value(instance.hydrogenForPower[n,h,i,w]),
										 dualVar]
							writer.writerow(my_string)
			if solver == 'Xpress':
				fError.write('\n')
					...
					for w in instance.Scenario:
						...
						for (s, h) in instance.HoursOfSeason:
							my_string = [n, inv_per[int(i - 1)], w, s, h,
										 ...
										 value(instance.hydrogenForPower[n,h,i,w]),
										 dualVar,
										 value(instance.hydrogenSoldOtherMarket[n,h,i,w])]
							writer.writerow(my_string)
			if solver == 'Xpress':
				fError.write('\n')
				fError.close()
		...
		writer = csv.writer(f)
		header = ['Period','Discounted electrolyzer cost [EUR]', 'Discounted pipeline cost [EUR]', 'Discounted storage cost [EUR]',
				  'Total discounted cost [EUR]', 'Hydrogen sold [kg]','Non-discounted price for NPV = 0 [EUR]']
		writer.writerow(header)
		if h2storage is True:
			for i in instance.Period:
				electrolyzerCost = value(sum(instance.discount_multiplier[j] * instance.elyzerInvCost[j] * sum(instance.elyzerCapBuilt[n,j] for n in instance.HydrogenProdNode) for j in instance.Period if j<=i))
				...
				my_string = [inv_per[i-1],
							 ...
							 pipelineCost,
							 storageCost,
							 electrolyzerCost + pipelineCost + storageCost]
				if value(sum(instance.hydrogenDemand[n,i] for n in instance.HydrogenProdNode)) > 0:
					my_string.extend([(electrolyzerCost + pipelineCost + storageCost)/value(sum(instance.discount_multiplier[i] * instance.hydrogenDemand[n,i] for n in instance.HydrogenProdNode))])
				else:
					my_string.extend([0])
				writer.writerow(my_string)
		else:
